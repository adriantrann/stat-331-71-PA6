...
```
After the transformation, the OLS line fits the data much better. We will fit a regression line to this transformed data. From here on out, we will examine only the relationship between $lGDP$ and $Life Expectancy$.
...
$$R^2_{\hat u^2} = 0.033$$
While the data isn't homoskedastic, the Breusch-Pagan test statistic shows that only 3.3% of the variance in $u_i$ can be explained by the variance in $lGDP$. We can say this because $var(u_i) = E(u^2_i) - E(u_i)^2$ and $E(u_i)=0$. While this heteroskedasticity is very much present with a p-value of 0, that is more a function of the huge sample size than a significant amount of the variation in $u^2$ being explained by $lGDP$. The relationship can be seen below. The blue regression line has a low $R^2$ of 0.033.
```{r}
