...
### 4. Homoskedasticity
Homoskedasticity means that the residuals at each point on the regression line have equal variance. In other words, the fitted values cannot predict the residuals. We conducted the Breusch-Pagan test to determine if Heteroskedasticity (unequal variances across the regression line) is present in the data.
```{r}
bptest(logmodel)
```
...
beta0+beta1
```
The left graph shows how the mean $Life Expectancy$ when GDP per capita is 1 dollar has increased dramatically over time. The right graph shows that while a 1% increase in GDP per capita increased $LifeExpectancy$ by almost 0.07 years in 1959, it now does so by only around 0.04 years.
